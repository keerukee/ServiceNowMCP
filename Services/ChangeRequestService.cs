...
/// <summary>
/// Service implementation for change_request table CRUD operations.
/// </summary>
public sealed class ChangeRequestService : IChangeRequestReader, IChangeRequestWriter
{
    private readonly IServiceNowHttpClient _client;
    private const string TableName = "change_request";
    private const string ApiBase = "api/now/table/" + TableName;

    public ChangeRequestService(IServiceNowHttpClient client)
    {
        _client = client;
    }
    ...
    {
        if (sysIdOrNumber.StartsWith("CHG", StringComparison.OrdinalIgnoreCase))
    ...
    public async Task<string> CreateAsync(object changeData)
    {
        return await _client.PostAsync(
            $"{ApiBase}?sysparm_display_value=all", changeData);
    }
